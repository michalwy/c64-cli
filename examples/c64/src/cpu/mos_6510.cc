...
void mos_6510::create_zero_page() {
	auto zp = harpoon::memory::make_io_memory<harpoon::memory::linear_random_access_memory>("ZPG $0000", harpoon::memory::address_range{0x0000, 0x00FF});
	add_component(zp);
	_zero_page = zp;

	zp->add_out_port(0x01, std::bind(&mos_6510::_zp_01_processor_port_out, this, std::placeholders::_1, std::placeholders::_2));
}
...
void mos_6510::init_zero_page() {
	...
	std::map<harpoon::memory::address, std::uint8_t> data = {
		{ 0x00, 0x2F },
		{ 0x01, 0x37 }
	};
